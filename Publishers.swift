...
class Publishers
{
    ...
    func indexPathForNewRandomPublisher() -> NSIndexPath{
        ...
        return indexPathForPublisher(publisher: newPublisher)
    }
    ...
    func indexPathForPublisher(publisher: Publisher) -> NSIndexPath
    {
        ...
        var item = 0
        for (index, currentPublisher) in publishersForSection(index: section!).enumerated() {
            if currentPublisher === publisher {
                item = index
                break
            }
        }
        //return NSIndexPath(item: item, inSection: section!)
        return NSIndexPath(item: item, section: section!)
    }
    
    func numberOfPublishersInSection(index: Int) -> Int {
        let publishers = publishersForSection(index: index)
        return publishers.count
    }
    
    func publisherForItemAtIndexPath(indexPath: NSIndexPath) -> Publisher? {
        if indexPath.section > 0 {
            let publishers = publishersForSection(index: indexPath.section)
    ...
    private func absoluteIndexForIndexPath(indexPath: NSIndexPath) -> Int
    {
        ...
        for i in 0..<indexPath.section {
            index += numberOfPublishersInSection(index: i)
        }
